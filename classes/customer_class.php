...
class customer_class extends db_connection
{
    ...
    public function get_customer_by_email($email_address)
    {
        ...
        $result = $this->db_fetch_one($search_query);
        
        // Ensure we return an array or false, not an object
    ...
    public function verify_login($email_address, $entered_password)
    {
        ...
        if ($customer_info && is_array($customer_info) && isset($customer_info['customer_pass'])) {
            if (password_verify($entered_password, $customer_info['customer_pass'])) {
    ...
    /**
     * Get customer by email address (enhanced for login)
     * Returns customer data if found, checks password match
     */
    public function login_customer($email_address, $entered_password)
    {
        // First find the customer by their email
        $customer_data = $this->get_customer_by_email($email_address);
        
        if (!$customer_data || !is_array($customer_data)) {
            // No customer found with this email or invalid data format
            return false;
        }
        
        // Check if the password matches what we have stored
        if (isset($customer_data['customer_pass']) && password_verify($entered_password, $customer_data['customer_pass'])) {
            // Password is correct! Remove sensitive data before returning
            unset($customer_data['customer_pass']);
